// based on this github project: https://github.com/tanisha-verma/lottie-web-react
// extra info can be seen here: https://airbnb.io/lottie/#/web?id=usage-1
...
const propTypes = {
  ...
  playSegments: PropTypes.shape({
    segments: PropTypes.arrayOf(
      PropTypes.shape({
        fromFrame: PropTypes.number,
        toFrame: PropTypes.number
      })
    ),
  ...
  const addEventListeners = useCallback(
    () => {
      if (lottieInstance && eventListeners) {
        eventListeners.forEach((eventListener) => {
          lottieInstance && eventListener.eventName && lottieInstance.addEventListener(eventListener.eventName, eventListener.callback)
        })
      }
    },
